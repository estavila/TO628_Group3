...
# Run Linear Regression excluding outliers (Iran). The output shows that 3 variables are statistically significant to Death Rate and the p-values are closing to '0'. Also, this could improve the Adjusted R-squared from 0.1615 to0.4889.
df_flat_2<-df_flat[!(df_flat$CountryName=="Iran"),]
df_flat_model_2<-lm(DeathRate ~ S1_School.closing+S2_Workplace.closing+S3_Cancel.public.events+S4_Close.public.transport+S5_Public.information.campaigns+S6_Restrictions.on.internal.movement+S7_International.travel.controls, data=df_flat_2 )
summary(df_flat_model_2)
residualPlots(df_flat_model_2)
  
  
# Linear Regression model without removing outliers. 3 variables are statistically significant, but only 1 of them are closing to '0', two others are greater than 0.01.
